...
class WixEcommerceClient extends WixCoreClient {
  ...
  private initializeWixClient(): void {
    try {
      // Note: SDK packages removed since WIX_SDK_CART_ENABLED = false
      this.wixClient = createClient({
        // modules: { currentCart, redirects }, // Removed - packages deleted
        auth: OAuthStrategy({
          clientId: this.clientId,
        }),
      });
      console.log('✅ [ECOMMERCE SDK] Wix SDK client initialized (cart/redirects modules removed)');
    } catch (error) {
  ...
  async getCurrentCart(): Promise<WixCart | null> {
    try {
      ...
      if (featureManager.isCartSDKEnabled() && this.wixClient) {
        console.log('🛒 [ECOMMERCE] Using Cart SDK');
        cart = await this.wixClient.currentCart.getCurrentCart();
      } else {
        console.log('🛒 [ECOMMERCE] Using REST API fallback');
        
        try {
          // REST API call to get current cart
          const response = await this.get('/ecom/v1/carts/current');
          cart = response.cart;
        } catch (error: any) {
          if (error?.status === 404 || error?.message?.includes('not found')) {
            console.log('ℹ️ [ECOMMERCE] No cart found via REST API');
            return null;
          }
          throw error;
        }
  ...
  async addToCart(items: WixCartItem[]): Promise<WixCart> {
    try {
      ...
      if (featureManager.isCartSDKEnabled() && this.wixClient) {
        ...
        // REST API call to add items to cart
        const response = await this.post('/ecom/v1/carts/current/line-items', {
          lineItems: lineItems
        });
  ...
  async updateCartItemQuantity(lineItemId: string, quantity: number): Promise<WixCart> {
    try {
      ...
      if (featureManager.isCartSDKEnabled() && this.wixClient) {
        ...
        // REST API call to update cart item quantity
        const response = await this.patch(`/ecom/v1/carts/current/line-items/${lineItemId}`, {
          quantity: quantity
        });
  ...
  async removeFromCart(lineItemIds: string[]): Promise<WixCart> {
    try {
      ...
      if (featureManager.isCartSDKEnabled() && this.wixClient) {
        ...
        // REST API call to remove cart items
        const response = await this.delete('/ecom/v1/carts/current/line-items', {
