...
import { bookingService, type ServiceQuery, type TimeSlot, type BookingRequest } from '../../screens/wix/services/shared/WixBookingService';
...
export const useServiceDetail = (serviceId: string, providerId?: string): UseServiceDetailReturn => {
  ...
  const loadService = useCallback(async () => {
    ...
    try {
      ...
      const wixService = await bookingService.getService(query);
      
      // Adapt the WixService to the generic Service format
      const adaptedService = adaptWixService(wixService, wixService.providers);

      if (mounted.current) {
        safeSetState(prev => ({
          service: adaptedService,
          providers: wixService.providers || [],
          loading: false,
          error: null,
          selectedProvider: providerId 
            ? wixService.providers?.find(p => p.id === providerId) || null 
            : wixService.providers?.[0] || null,
        }));
