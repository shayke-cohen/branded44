...
const getClientId = () => 'mock-client-id';
const getSiteId = () => 'mock-site-id';
const getApiBaseUrl = () => 'https://mock-api.wix.com';
  ...
  async queryServices(filters?: {
    ...
    try {
      ...
      if (isBasicQuery && !filters?.forceRefresh) {
        ...
        if (cached) {
          console.log('✅ [BOOKING CACHE] Using cached data for wix_services_cache');
          return cached;
        }
      } else if (filters?.forceRefresh) {
        console.log('🔄 [BOOKING CACHE] Force refresh requested, bypassing cache');
      }
      ...
      console.log('📅 [BOOKING] Raw services response:', JSON.stringify(response, null, 2));
