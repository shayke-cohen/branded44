...
import { restaurantService, RestaurantOrder } from '../shared/WixRestaurantService';
...
export const useRestaurant = (): UseRestaurantReturn => {
  ...
  const refreshData = useCallback(async (restaurantId?: string) => {
    try {
      ...
      await Promise.all([
        ...
        loadPopularItems(restaurantId),
        loadFeaturedItems(restaurantId),
      ]);
   ...
   */
  useEffect(() => {
    if (mounted.current && !state.restaurant && !state.loading) {
      refreshData();
    }
